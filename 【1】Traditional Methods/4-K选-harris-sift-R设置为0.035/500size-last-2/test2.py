...
def main_workflow():
    ...
    for current_k in K_VALUES:
        ...
        try:
            ...
            results['RAM_Usage'].append(ram_usage)
...
def plot_results(results):
    ...
    # 3. 资源消耗曲线
    plt.figure(figsize=(12, 6))
    plt.plot(df['K'], df['CPU_Usage'], label='CPU Usage (%)', marker='o')
    plt.plot(df['K'], df['RAM_Usage'], label='RAM Usage (%)', marker='s')
    ...
    # 5. 准确率分析曲线
    plt.figure(figsize=(12, 6))
    plt.plot(df['K'], df['Train_Accuracy'], 
    ...
    plt.plot(df['K'], df['Test_Accuracy'], 
             label='Test Accuracy',
             marker='s', 
             color='#d7191c',
             linewidth=2.5,
    ...
    plt.scatter(df.loc[best_idx, 'K'], 
                df.loc[best_idx, 'Test_Accuracy'],
                s=200, 
                facecolors='none', 
                edgecolors='#ff7f00',
                linewidths=2,
    ...
    plt.annotate(f'Max Test Acc: {df.loc[best_idx, "Test_Accuracy"]:.3f}',
                 xy=(df.loc[best_idx, 'K'], df.loc[best_idx, 'Test_Accuracy']),
                 xytext=(20, 20),
    ...
    plt.title('Accuracy Analysis by Visual Dictionary Size')
    plt.xlabel('Visual Dictionary Size (K)')
    plt.ylabel('Accuracy')
    plt.ylim(0.3, 1.05)
    ...
    trend_text = [
        "Key Observations:",
        "1. Training accuracy consistently improves with larger K",
        "2. Test accuracy peaks at K=%d" % df.loc[best_idx, 'K'],
        "3. Overfitting becomes evident when K > %d" % df.loc[best_idx, 'K']
    ]
    plt.text(0.02, 0.18, '\n'.join(trend_text),
    ...
    plt.savefig('analysis/accuracy_curve.pdf', 
                bbox_inches='tight', 
                dpi=300)
    plt.close()
